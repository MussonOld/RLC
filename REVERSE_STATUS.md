...
- `GPIOA` маска `0x2` (бит 1, **`PA1` — сигнал не идентифицирован в
  `GPIO_MAP.md`**, открытый вопрос) — debounce 10 тиков, на удержании
  вызывает `FUN_0800ee00(0)` (автовыключение/выключение по кнопке,
  причина `0`);
...
(в т.ч. ручное выключение через удержание `PA1` -> `FUN_0800ee00(0)`)**.
Не выяснено: реальная частота тика TIM7 (нужны PSC/ARR, раздел 11), имя
сигнала `PA1`, назначение блока C.
...
13. Новые открытые вопросы из раздела 6.3: идентифицировать сигнал `PA1` (используется как кнопка выключения); разобрать `FUN_08005148`/`VectorUnsignedToFloat`, чтобы установить физический смысл аналогового канала блока C; разобрать `thunk_EXT_FUN_10000bc0` (вероятно buzzer) по байтам в CCM RAM.
...
- РАЗОБРАН ЦЕЛИКОМ обработчик TIM7 (`FUN_0800ff78`, раздел 6.3).
  ...
  и периодический дебаунс трёх кнопок/джойстика (`PA1` — не
  идентифицирован, `PA2`=`JOY_C`, `PB12`=`JOY_B`), плюс отдельный
  аналоговый канал сравнения с порогом (физический смысл не установлен).
  Новые открытые вопросы зафиксированы в разделе 13, пункт 13.
